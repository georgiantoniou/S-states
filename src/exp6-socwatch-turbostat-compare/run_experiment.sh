...
start_measurements () {
    ...
    sleep 10
    ../measurements/run_profiler.sh start_measurements $PASSWD $USERNAME $HOST
    python3 ../measurements/pdu.py $USERPDU $PASSWDPDU $HOSTPDU $2 &> $1/"pdu_"$3"_"$2".out" &
    ../measurements/turbostat_residency.sh main 12 10 $USERNAME $PASSWD $HOST &
    #socwatch_output="socwatch_"$3"_"$2
    #socwatch_log="socwatch_"$3"_"$2".log"
    #../measurements/socwatch_measurements.sh main "-l $socwatch_log" "-o $socwatch_output" "-f hw-cpu-cstate" 120 $USERNAME $PASSWD $HOST
...
report_measurements () {
    ...
    #../measurements/socwatch_measurements.sh report_measurements $PASSWD $USERNAME $HOST ~/"socwatch_"$2"_"$3".csv" &> $1/"socwatch_"$2"_"$3".csv"
...
main () {
    ...
    if [[ -z "$1" || -z "$2" || -z "$3" || -z "$4" || -z "$5" || -z "$6" || -z "$7" || -z "$8" || -z "$9" || -z "$10" ]]; then
        ...
        echo "***SYNTAX: ./run_experiment main benchmarkname #runs executiontime resultdir machine conf runcommand startcore endcore runcommandend"
        exit;   
    fi 
    ...
    do
        ...
        if [ "$benchmark" != "idle" ]; then
            ...
            ./run_benchmark.sh start_benchmarks "$run_command" $start_core $end_core "$run_command_end"
